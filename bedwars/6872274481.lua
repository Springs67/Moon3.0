...
runcode(function()
	...
	MovementSection:NewToggle("Speed - Mode 1", "cframe mode 1!1!1", function(enabled)
		if enabled then
			...
			SpeedRepeat = true
			repeat wait()
				lplr.Character.Humanoid.WalkSpeed = speed1
...
if _G.DeveloperMode == true then
	runcode(function()
		local TeleportAmount1 = 15
		local TeleportSpeed1 = 0.5
		local AcBypassEnabled = false
		local TeleportAmount3 = 0.1
		MovementSection:NewToggle("AnticheatBypass", "allows you to bypass the anticheat speed check", function(enabled)
			if enabled then
				...
				local function yess()
					repeat
						task.wait(TeleportSpeed1)
						oldchar:SetPrimaryPartCFrame(clone.PrimaryPart.CFrame)
					until not AcBypassEnabled
				end
				coroutine.wrap(yess)()
				local function yess2()
					repeat
						wait()
						if uis:IsKeyDown(Enum.KeyCode.W) then
							oldchar.HumanoidRootPart.CFrame = oldchar.HumanoidRootPart.CFrame + oldchar.HumanoidRootPart.CFrame.lookVector * TeleportAmount3
						elseif uis:IsKeyDown(Enum.KeyCode.S) then
							oldchar.HumanoidRootPart.CFrame = oldchar.HumanoidRootPart.CFrame + oldchar.HumanoidRootPart.CFrame.lookVector * TeleportAmount3
						end
					until not AcBypassEnabled
				end
				coroutine.wrap(yess2)()
				cam.CameraSubject = clone.Humanoid 
				clone.Parent = workspace
				lplr.Character = clone
			else
				oldchar.Humanoid.Health = 0
				AcBypassEnabled = false
			end
		end)
		MovementSection:NewTextBox("TeleportAmount", "recommended 14", function(TeleportAmount2)
			TeleportAmount1 = TeleportAmount2
		end)
		MovementSection:NewTextBox("TeleportSpeed", "recommended 1.3", function(TeleportSpeed2)
			TeleportSpeed1 = TeleportSpeed2
		end)
		MovementSection:NewTextBox("TeleportAmount2", "recommended 0.22", function(TeleportAmount4)
			TeleportAmount3 = TeleportAmount4
		end)
	end)
end
